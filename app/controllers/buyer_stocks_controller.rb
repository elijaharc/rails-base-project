  ...
  def create
    ...
    if existing_stock
      if params[:stock][:quantity].to_i < 1
        ...
        @broker_transaction = Transaction.create(broker_id: stock.added_by, broker_name: stock.brokers.first.broker_name, buyer_email: current_buyer.email, stock_id: stock.id, buyer_id: current_buyer.id, ticker: stock.ticker, company_name: stock.name, stock_price: stock.last_price, quantity: params[:stock][:quantity])
